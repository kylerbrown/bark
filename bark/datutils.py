...
class Stream():
    ...
    def __next__(self):
        ...
        try:
            result = self.data[self.index: self.index
                    + self.chunksize]
        except IndexError:
            raise StopIteration
        print(result)
    ...
    def new_stream(self, newdata):
        ...
        return Stream(newdata, sr=self.sr,
                attrs=self.attrs,
    ...
    def _binary_operator(self, other, func):
        if isinstance(other, Stream):
           return self.new_stream( (func(x, y)
               for x,y in zip(self, other)))
        else:
            return self.new_stream((func(x, other)
                for x in self))
