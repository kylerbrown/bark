...
import numpy as np
from bark.io.openephys.kwik import load_all
...
def filename_to_timestamp(fname, timezone):
    return arrow.get(fname, 'YYYY-MM-DD_HH-mm-ss').replace(
...
def kwd_to_entry():
    ...
    p.add_argument("-o", "--out", help="name and path of output bark entry")
    p.add_argument("-t",
                   "--timestamp",
                   help="""format: YYYY-MM-DD or YYYY-MM-DDTHH:MM:SS.S,
        if left unspecified
            the timestamp will be inferred from
            the foldername of the kwd file.""")
    p.add_argument("-a",
                   "--attributes",
                   action='append',
    ...
    eofolder2entry(args.kwdfolder, args.out, args.timestamp, args.timezone,
                   **attrs)
