...
def test_points_to_span():
    n_samples = 31
    def common(pts, ns):
        output = bs.points_to_span(pts, ns)
...
def test_label_to_splits():
    ...
    start_samples = [7, 19, 25, 29]
    start_times = [x / sattrs['sampling_rate'] for x in start_samples]
    edata = pd.DataFrame({'start': start_times,
                          'name': ['a', 'b', 'a', 'c']})
    eattrs = {'units': 's'}
    ...
    stop_samples = [10, 20, 28, 31]
    edata['stop'] = [x / sattrs['sampling_rate'] for x in stop_samples]
    event = bark.EventData(edata, path, eattrs)
    ...
    assert output['stop'][0] == 10
    assert output['name'][0] == event.data['name'][0]
    assert output['start'][1] == 25
    assert output['stop'][1] == 28
